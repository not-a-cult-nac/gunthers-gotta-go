...
class AIController {
    ...
    decideInCar(state, inputs, reactionComplete) {
        ...
        // Drive toward goal until we win
        const goalDistZ = GOAL_Z - car.z;  // Positive = goal ahead, negative = overshot
        const goalDistTotal = Math.hypot(car.x, car.z - GOAL_Z);
        
        // Steer toward center, harder when close
        const steerMultiplier = goalDistTotal < 20 ? 0.1 : 0.02;
        const steerAmount = Math.max(-1, Math.min(1, -car.x * steerMultiplier));
        
        // Drive forward if goal ahead, reverse if overshot
        inputs.drive = goalDistZ >= 0 ? 1 : -1;
